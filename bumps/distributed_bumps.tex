...
\section{Intractability of the Global Optimum}

Even with a perfect oracle that provides the exact geometry of all $k$ bumps, calculating the global optimum directly is an NP-Hard combinatorial subset selection problem (requiring a search over all $2^k$ possible subsets of intersecting ridges to minimize the global weight-decay penalty). For the full mathematical derivation of this overparameterized trade-off, we direct the reader to the corresponding section in the companion paper on the bump data-generating process. This fundamental intractability further motivates the need for a distributed, swarm-like search to dynamically explore promising subsets of bump intersections.
...
\section{Data Order and Chaotic Trajectories}

A core assumption of our proposal is that workers will naturally fan out to explore the landscape independently. In a deterministic setting, initializing all workers with identical (or nearly identical) weights would cause them to collapse into a single trajectory immediately. However, empirical studies of real neural networks reveal that SGD dynamics exhibit an extreme ``butterfly effect'' early in training \cite{altintas2025butterfly}. 

Because the landscape is highly non-convex and chaotic, simply randomizing the order of the mini-batches is sufficient to cause identical models to diverge exponentially fast into completely orthogonal subspaces or distinct basins of attraction \cite{fort2019deep}. This means that we do not necessarily need to forcefully disperse the workers at initialization; the inherent mini-batch noise and data shuffling provide more than enough stochasticity to guarantee independent exploration.

Furthermore, this dynamic provides a critical statistical advantage: because the workers are driven by different random sequences of data, their individual trajectories are fundamentally out-of-sample with respect to one another. When the soft consensus mechanism pulls a worker toward a newly discovered feature, it is pulling it toward a structure verified by a \emph{different} subset of the data stream. By aggregating these effectively out-of-sample discoveries, the grand average $\bar{\theta}$ acts as a regularizer, confirming that a bump is a genuine feature of the data distribution rather than an artifact of a single worker over-fitting to its specific mini-batch sequence.
...
The bump model also points to several nearby algorithms that should be
studied side by side with consensus-coupled SGD.

\paragraph{Stochastic Gradient Langevin Dynamics.}
SGLD \cite{welling2011bayesian} adds explicit injected noise to
stochastic gradient descent. In the bump landscape, this gives a direct
way to test the thermodynamic story from the original paper: broad bumps
should retain mass while sharp bumps should lose it more easily as the
noise level rises. SGLD is therefore a natural baseline for asking
whether our consensus mechanism is doing something beyond simple
temperature control.

\paragraph{Parallel tempering and replica exchange.}
Parallel tempering \cite{earl2005parallel} attacks multimodality by
running replicas at different temperatures and occasionally swapping
states. This is conceptually close to our setting because it also uses a
population of coupled trajectories to move between separated basins. The
key difference is that replica exchange communicates through occasional
state swaps across temperatures, while our method communicates through a
continuous consensus field at one temperature. The bump model should let
us compare these mechanisms directly on the same multimodal geometry.

\paragraph{Randomized gossip and local consensus.}
Randomized gossip algorithms \cite{boyd2006randomized} replace the
global average with repeated local averaging on a communication graph.
This may be a better fit than a single global center when the ensemble
should preserve several semi-independent subpopulations. In the bump
model, gossip would let us test whether discoveries should propagate
through local neighborhoods first rather than being broadcast through one
global mean. It also makes the spectral-gap control explicit, since the
mixing of the communication graph determines the speed of consensus.

\paragraph{Particle swarm optimization as a foil.}
Particle swarm optimization \cite{kennedy1995particle} is the classic
population method for multimodal search. It is worth including not
because it is likely to dominate in high-dimensional bump landscapes,
but because it isolates what the gradient contributes. PSO has the
population-level communication and attraction mechanisms, but lacks the
first-order directional information that lets SGD slide rapidly along a
low-rank ridge. On the bump model, that difference should be unusually
clean.

These algorithms are not replacements for the present proposal; they are
comparison points. The bump model is useful precisely because it can
separate several distinct ingredients: gradient guidance, explicit
thermal noise, pairwise or graph-based communication, and global
consensus. A good experimental program should vary these ingredients one
at a time rather than treating distributed optimization as a single
monolithic design choice.

\section{Further Connections to the Literature}

\subsection*{Primary discussion}

\paragraph{FedAvg, local SGD, and post-local SGD.}
The most immediate connection is to communication-efficient distributed
training. Federated averaging (FedAvg) \cite{mcmahan2017communication}
and local SGD \cite{stich2019local} reduce synchronization by allowing
workers to take multiple local steps before averaging. Post-local SGD
\cite{lin2020dont} is especially close in spirit to our story because it
already separates training into phases: a more coupled early regime and
a more local later regime. SlowMo \cite{wang2020slowmo} goes further by
adding a slow outer momentum to periodic synchronization. These papers
should be part of the primary discussion because our proposal is not
just an abstract particle system; it is a claim about how distributed
SGD should be run when discovery of multiple structures matters.

\paragraph{Consensus-based optimization.}
Consensus-based optimization (CBO) \cite{pinnau2017consensus} provides
the closest mathematical relative outside deep-learning practice. CBO
studies interacting particles that contract toward a consensus point
while stochastic exploration resists premature collapse. It should be a
primary discussion because it offers mean-field language, particle
interpretations, and convergence questions that are directly relevant to
our consensus field. The main distinction is that the distributed bump
proposal is first-order and feature-directed, whereas standard CBO is
largely zero-order.

\subsection*{Annotated secondary connections}

\paragraph{Deep ensembles.}
Deep ensembles \cite{lakshminarayanan2017simple} are worth citing
because they validate the value of maintaining multiple distinct
solutions, but they are not a primary discussion because they focus on
prediction quality and uncertainty after training, not on coupled
exploration during training.

\paragraph{Stochastic weight averaging and model soups.}
SWA \cite{izmailov2018averaging} and model soups
\cite{wortsman2022model} matter because they describe when weight-space
averaging is beneficial, but they are secondary here because they are
mainly late-stage consolidation methods rather than mechanisms for
discovering separated bumps.

\paragraph{Entropy-SGD and SAM.}
Entropy-SGD \cite{chaudhari2017entropy} and sharpness-aware
minimization \cite{foret2021sharpness} connect to our broad-versus-sharp
story, but they are not the primary comparison because they reshape a
single trajectory rather than coordinating a population of separated
workers.

\paragraph{Mode connectivity.}
Mode connectivity \cite{garipov2018loss} is relevant because it helps
explain when averaging or interpolation between solutions is safe, but
it is a secondary discussion because it says more about the geometry
after multiple solutions have been found than about the discovery
mechanism itself.
...
Because the bump data-generating process has a closed-form geometric definition, we can analytically estimate this spectral gap $1-\lambda_2$ and derive a concrete tuning rule for the coupling strength $\gamma$.

When a worker has successfully anchored onto the ridge of a bump with height $b$ and center $c=0$ (without loss of generality), it has found the optimal coordinate for the signal directions. We are interested in the dynamics of the orthogonal, irrelevant directions $\theta_{\perp}$. By definition, in these irrelevant directions, the shape matrix $A_i$ has zero eigenvalues. Therefore, the bump term contributes a constant height $b$, and the local loss function $f$ simplifies to the global regularizer term:
\[
f(\theta_{\perp}) \approx b - \lambda \|\theta_{\perp}\|^2
\]
Since SGD is performing gradient \emph{ascent} to find the peak of this landscape, the gradient in the irrelevant subspace is exactly linear:
\[
\nabla_{\perp} f(\theta_{\perp}) = -2\lambda \, \theta_{\perp}
\]
The SGD update step in these irrelevant directions is therefore a perfect, linear discrete-time Ornstein-Uhlenbeck (AR(1)) process:
\[
\theta_{\perp}^{t+1} = \theta_{\perp}^t + \eta \nabla_{\perp} f(\theta_{\perp}^t) + \xi_t
\]
\[
\theta_{\perp}^{t+1} = (1 - 2\eta \lambda) \theta_{\perp}^t + \xi_t
\]
where $\xi_t$ is the mini-batch gradient noise. For an AR(1) process with decay coefficient $\rho = 1 - 2\eta \lambda$, the eigenvalues of the transition operator $P_{\perp}$ are simply $\lambda_k = \rho^k$. Therefore, the second eigenvalue, which governs the slowest mode of decorrelation, is:
\[
\lambda_2 = 1 - 2\eta \lambda
\]
The spectral gap is precisely $1-\lambda_2 = 2\eta \lambda$. This gives us an exact, closed-form estimate for the independent-draw time:
\[
\tau_{\mathrm{ind}} \approx \frac{1}{1-\lambda_2} = \frac{1}{2\eta \lambda}
\]
...
\[
\gamma \approx (2 \log 2) \, \eta \lambda \approx 1.38 \, \eta \lambda
\]
This is a beautiful and highly practical result. It demonstrates that the optimal consensus coupling $\gamma$ should be strictly proportional to the local learning rate $\eta$ and the global regularization constant $\lambda$. Importantly, because we are analyzing the orthogonal directions, neither the condition number of the shape matrix $A_i$ nor the height of the bump $b$ enter this equation. The required coupling strength is dictated entirely by the global geometric ``gravity'' ($\lambda \|x\|^2$) holding the worker to the center of the space, completely decoupling the consensus tuning from the local features of the specific bump.

\section{Cool-down Dynamics: Coalescence vs.\ Ensembling}

A critical question for any distributed optimization algorithm is what happens at the end of training during the ``cool-down'' phase, when the learning rate $\eta$ is decayed toward zero to reduce mini-batch noise and achieve convergence. Does the ensemble collapse into a single unified model, or does it remain diverse?

We can answer this by examining the deterministic equilibrium state of the workers. As the mini-batch noise vanishes, a worker comes to rest when its expected update is zero:
\[
\Delta \theta_i = -\eta \nabla L(\theta_i) - \gamma(\theta_i - \bar{\theta}) = 0
\]
Rearranging this yields the steady-state displacement of worker $i$ from the grand average:
\[
\theta_i - \bar{\theta} = -\frac{\eta}{\gamma} \nabla L(\theta_i)
\]
This equilibrium equation reveals a fundamental choice in how the algorithm is tuned during the cool-down phase:

\textbf{Scenario A: Forced Coalescence (The Averaging Trap).} If one decays the learning rate ($\eta \to 0$) but holds the consensus coupling constant ($\gamma > 0$), the ratio $\frac{\eta}{\gamma} \to 0$. The equilibrium equation dictates that $\theta_i - \bar{\theta} \to 0$. The system forces all workers to perfectly coalesce into a single point. If the workers are situated in distinct, parallel bumps, this forced collapse will violently yank them out of their respective basins and deposit them in the empty, high-loss plateau at the true mathematical average. The system falls into the Averaging Trap precisely at the end of training.

\textbf{Scenario B: Persistent Diversity (Proportional Annealing).} However, our analytic derivation in the previous section established that the optimal tuning rule is $\gamma \propto \eta \lambda$. If we follow this geometric principle, then as we decay the learning rate $\eta \to 0$, we must \emph{simultaneously and proportionally} decay the consensus coupling $\gamma \to 0$. 

Under proportional annealing, the ratio $\frac{\eta}{\gamma}$ remains strictly constant. The equilibrium displacement $\theta_i - \bar{\theta}$ therefore remains finite. As the ``temperature'' of the system drops to zero, the workers will not naively collapse into the empty center. Because the consensus spring $\gamma$ weakens at the exact same rate as the local gradient step $\eta$, the local gradients continue to overpower the shrinking consensus pull, keeping the workers safely anchored on their distinct ridges. 

However, they do not merely freeze. If the ridges they occupy intersect, the combined pull of the local gradients and the consensus mean will cause the workers to slowly drift along their respective ridges toward that intersection. 

Furthermore, this dynamic exposes the nature of the global decay term. If the global geometric ``gravity'' ($\|x\|^2$) is not an intrinsic property of the data-generating process, but rather an artificial regularizer (e.g., explicit weight decay) added by the practitioner, it too can be annealed. If weight decay is annealed to zero alongside $\eta$ and $\gamma$, the penalty for moving far from the origin vanishes. Without this penalty, the intersection of the ridges strictly becomes the global optimum. The workers will invariably drift along their ridges until they coalesce exactly at this high-rank intersection, successfully assembling the disjoint discoveries into a single unified model without ever falling into the Averaging Trap.

This guarantees that the local optimization process is never derailed by the global consensus. Depending on whether weight decay is annealed or held constant, the final output of the distributed system can be precisely engineered to yield either a single, highly-structured collapsed model, or a geometrically robust Deep Ensemble of distinct models.
