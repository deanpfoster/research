...
Under proportional annealing, the ratio $\frac{\eta}{\gamma}$ remains strictly constant. The equilibrium displacement $\theta_i - \bar{\theta}$ therefore remains finite. As the ``temperature'' of the system drops to zero, the workers will never coalesce. Because the consensus spring $\gamma$ weakens at the exact same rate as the local gradient step $\eta$, the local gradients continue to overpower the shrinking consensus pull. The workers stay safely anchored in the distinct parallel bumps they discovered, simply freezing in place slightly offset toward the center of mass. 

This guarantees that the local optimization process is never derailed by the global consensus. The final output of the distributed system is not a single collapsed model, but rather a geometrically robust Deep Ensemble of distinct models, each occupying a different meaningful feature of the landscape.
