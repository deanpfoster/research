...
This mirrors the concept of the \emph{critical batch size} formally derived by Jain, Kakade, et al.\ \cite{jain2018parallelizing} for SGD on least squares. In their analysis of purely convex quadratics, increasing the batch size linearly reduces the stationary variance up to a critical threshold dictated by the inverse condition number and the noise profile. Beyond this critical batch size, parallel workers provide diminishing returns \cite{mccandlish2018empirical, shallue2019measuring}.

However, the martingale swarm exhibits a far more severe structural penalty. For standard data-parallelism, exceeding the critical batch size merely wastes compute. But for the martingale swarm, exceeding the \textbf{critical swarm size} $n_{\text{crit}} \approx 2\eta \lambda_{\min} / \gamma^2$ fundamentally breaks the algorithm. If $n$ exceeds this threshold, the injected communication noise required for exploration physically overwhelms the regularizer's restoring force, and the search process undergoes a catastrophic geometric explosion.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{fig_martingale_1d.pdf}
  \caption{Anisotropic temperature from martingale coupling. Top row:
    projection onto the anchor direction. Bottom row: projection onto the
    orthogonal direction. Left column: uncoupled. Right column: coupled.
    Coupling increases the variance along the anchor axis by $4.6\times$
    while leaving the orthogonal axis essentially unchanged ($1.2\times$).
    The measured boost is below the $10\times$ theoretical prediction because
    the agent's large excursions in the anchor direction carry it into
    regions where the linearization $F \approx \gamma S c$ breaks down.}
