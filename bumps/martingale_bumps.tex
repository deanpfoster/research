...
\paragraph{Communication Efficiency via Hamming Codes.}
While the full pairwise interaction seems to demand $O(n^2)$ dense communication, the sign matrix structure allows for elegant compression. If the signs $S_{ij}^t$ are structured deterministically using orthogonal columns (e.g., Walsh-Hadamard matrices) or error-correcting codes (e.g., a Hamming code basis), the swarm can compute the pairwise forces without sending $n$ distinct parameter vectors. The workers can simply broadcast $O(\log n)$ distinct linear combinations (parity sums) of their parameters. Agent $i$ then reconstructs its specific $F_i^t$ by adding or subtracting the appropriate $\log n$ broadcasted sums. This scales the communication bottleneck from $O(n)$ full parameter vectors per worker to $O(\log n)$, making massive swarms feasible in the $d \sim 10^{12}$ regime.
